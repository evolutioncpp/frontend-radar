...
Создайте локальные env-файлы. Выберите блок команд для вашей оболочки.
...
Web-приложение построено по принципам Feature-Sliced Design:

```text
apps/web/src/
  app/       провайдеры приложения, store, layouts и глобальные стили
  pages/     route-level страницы
  widgets/   составные блоки страниц
  features/  пользовательские сценарии и feature-логика
  entities/  доменные сущности и API adapters
  shared/    переиспользуемые UI, libs, config и низкоуровневая инфраструктура
```

Backend разделён по слоям: application, domain, analysis, scoring, localization и infrastructure.
